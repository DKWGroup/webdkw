...
const MarkdownEditor: React.FC<MarkdownEditorProps> = ({
  ...
  const formatText = (type: string) => {
    ...
    switch (type) {
      ...
      case 'table':
        ...
        break;
      case 'cta':
        insertText('[CTA:title=Kliknij tutaj,url=https://example.com,color=orange]');
        break;
      case 'tldr':
        insertText('\n\n## TL;DR\n\n');
        break;
  ...
  const toolbarButtons: ToolbarButton[] = [
    { icon: <Heading1 size={18} />, title: 'Nagłówek 1', action: () => formatText('heading1') },
    { icon: <Heading2 size={18} />, title: 'Nagłówek 2', action: () => formatText('heading2') },
    { icon: <Heading3 size={18} />, title: 'Nagłówek 3', action: () => formatText('heading3') },
    { icon: <Bold size={18} />, title: 'Pogrubienie', action: () => formatText('bold') },
    { icon: <Italic size={18} />, title: 'Kursywa', action: () => formatText('italic') },
    { icon: <List size={18} />, title: 'Lista', action: () => formatText('list') },
    { icon: <ListOrdered size={18} />, title: 'Lista numerowana', action: () => formatText('ordered-list') },
    { icon: <Quote size={18} />, title: 'Cytat', action: () => formatText('quote') },
    { icon: <Code size={18} />, title: 'Kod', action: () => formatText('code') },
    { icon: <Link size={18} />, title: 'Link', action: () => formatText('link') },
    { icon: <Image size={18} />, title: 'Obraz', action: () => formatText('image') },
    { icon: <Table size={18} />, title: 'Tabela', action: () => formatText('table') },
  ];
  ...
  return (
    <div className="flex flex-col border border-gray-300 rounded-lg overflow-hidden">
      ...
      <div className="bg-gray-50 border-b border-gray-300 p-2 flex flex-wrap items-center gap-1">
        ...
        <button
          ...
          className={`p-2 rounded transition-colors ${
            showHelp ? 'text-blue-600 bg-blue-100' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-200'
          }`}
          title="Pomoc"
        >
          <HelpCircle size={18} />
        ...
        <button
          ...
          className={`p-2 rounded transition-colors ${
            showPreview ? 'text-green-600 bg-green-100' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-200'
          }`}
      ...
      {showHelp && (
        <div className="bg-blue-50 p-4 text-sm border-b border-blue-200">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            <div><code># Tekst</code> - Nagłówek 1</div>
            <div><code>## Tekst</code> - Nagłówek 2</div>
            <div><code>### Tekst</code> - Nagłówek 3</div>
            <div><code>**Tekst**</code> - Pogrubienie</div>
            <div><code>*Tekst*</code> - Kursywa</div>
            <div><code>[Tekst](url)</code> - Link</div>
            <div><code>![alt](url)</code> - Obraz</div>
          ...
          <p className="mt-2 text-blue-700">
            <strong>Specjalne znaczniki:</strong> [CTA:title=Tekst,url=URL,color=kolor] - przycisk CTA
          </p>
        </div>
      )}
      ...
      <div className="bg-gray-50 border-t border-gray-300 px-3 py-1 text-xs text-gray-500 flex justify-between">
        <div>
          {markdown.length} znaków | {markdown.split(/\s+/).filter(Boolean).length} słów
        </div>
        <div>
