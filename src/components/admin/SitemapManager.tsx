...
const SitemapManager: React.FC = () => {
  ...
  const checkExistingSitemap = async () => {
    ...
    try {
      ...
      console.error('Error checking existing sitemap:', err);
      setError('Nie udało się sprawdzić istniejącej mapy witryny');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGenerateComplete = (url: string) => {
    ...
    setSuccessMessage('Mapa witryny została pomyślnie wygenerowana!');
  ...
  const submitToSearchEngines = async () => {
    ...
    try {
      ...
      setSuccessMessage('Wyszukiwarki zostały powiadomione o nowej mapie witryny!');
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h2 className="text-xl font-bold text-gray-900 mb-4">Zarządzanie mapą witryny</h2>
        ...
        <div className="flex items-start space-x-3">
          ...
          <div>
            ...
            <ul className="text-sm text-blue-800 space-y-1">
              <li>Generuj nową mapę witryny po dodaniu nowych artykułów lub stron</li>
              <li>Powiadamiaj wyszukiwarki o aktualizacjach, aby przyspieszyć indeksowanie</li>
              <li>Dodaj odnośnik do mapy witryny w pliku robots.txt</li>
              <li>Sprawdzaj regularnie status indeksowania w Google Search Console</li>
            </ul>
          </div>
        </div>
      </div>
